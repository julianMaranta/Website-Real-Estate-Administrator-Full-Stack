    ...
    <div class="content">
      ...
      <form @submit.prevent="createProperty">
        ...
        <button 
          ...
          class="submit-btn"
        >
          {{ uploading ? 'Subiendo imágenes...' : 'Publicar Propiedad' }}
        </button>
      </form>
    </div>
...
const uploadImageToDropbox = async (image: UploadedImage): Promise<string> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const uniqueFileName = `propiedad_${propertyId}_${timestamp}_${randomString}.${fileExtension}`;
      const dropboxPath = `/propiedades/${propertyId}/${uniqueFileName}`;
